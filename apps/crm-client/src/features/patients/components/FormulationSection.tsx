...
export const FormulationSection: React.FC<FormulationSectionProps> = ({
  title,
  optionsKey,
  fieldKey,
  initialData,
  ...
  const computeInitialData = useCallback(() => {
    ...
    if (!data.text && patient.diagnosis) {
      ...
      if (fieldKey === 'synthesis') {
        template =
          `Paciente de ${patient.age} años con diagnóstico principal de ${patient.diagnosis}.` +
          (patient.pathologyType
            ? ` Se clasifica dentro del espectro: ${PATHOLOGY_MAP[patient.pathologyType] || patient.pathologyType}.`
            : '');
  ...
  const [isDirty, setIsDirty] = useState(
    (initialComputed as FormulationData & { isAutoGenerated?: boolean }).isAutoGenerated || false,
  );
