...
export const NeonIcon: React.FC<NeonIconProps> = ({
  ...
  glowColor = '#3b82f6', // Corporate Blue default (was Cyan)
  size = 'md',
  bgClass = 'bg-slate-950', // Corporate Dark default
  className = '',
}) => {
  // Size Mappings
  ...
  return (
    <div className={`relative group ${containerSizes[size]} ${className}`}>
      ...
      <div
        className={`relative w-full h-full rounded-full ${bgClass} border border-white/10 flex items-center justify-center shadow-[0_0_15px_-3px_rgba(0,0,0,0.5)] group-hover:border-white/30 transition-all duration-500 overflow-hidden`}
      >
