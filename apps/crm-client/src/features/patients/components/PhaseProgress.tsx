        ...
        <div className="mb-8 flex justify-between items-start">
          ...
          <div className="flex flex-col items-end">
            <div className="flex items-baseline gap-1">
              <span
                className={`text-3xl font-black ${adherence >= 80 ? 'text-emerald-600' : adherence >= 60 ? 'text-amber-500' : 'text-red-500'}`}
              >
                {adherence}%
              </span>
        ...
        <div className="relative px-4">
          ...
          <div className="grid grid-cols-5 gap-2 relative">
            {TREATMENT_PHASES.map((phase) => {
              ...
              let circleClasses =
                'w-10 h-10 rounded-full flex items-center justify-center border-4 z-10 transition-all duration-500 mb-3 ';
              if (isCompleted) {
              ...
              return (
                <div key={phase.id} className="flex flex-col items-center group">
                  ...
                  <div className="text-center space-y-1">
                    <p
                      className={`text-xs font-bold uppercase tracking-wider transition-colors duration-300 ${textClass}`}
                    >
                      {phase.name.split(' ')[0]}
                    </p>
                    ...
                    {isActive && (
                      <span
                        className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-bold mt-1 ${phase.bg} ${phase.text}`}
                      >
                        En Curso
                      </span>
                    )}
