...
test.describe('Patient Lifecycle Check', () => {
  ...
  test('should navigates to Patients and handle New Patient action (Create or Paywall)', async ({
    page,
  }) => {
    ...
    if (await paywallHeader.isVisible()) {
      ...
      throw new Error(
        '❌ Critical: Modal backdrop appeared but neither Paywall nor Create Header was found.',
      );
    }
  });
});
