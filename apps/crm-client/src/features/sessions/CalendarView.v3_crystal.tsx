...
export const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  // 1. DATA AGGREGATION & NORMALIZATION (The "Brain")
  const allEvents = useMemo(() => {
    // A. Individual Sessions (From Patients)
    const individuals = patients.flatMap((patient) =>
      (patient.sessions || []).map((session) => ({
        ...
        status: session.isAbsent ? 'absent' : 'scheduled',
        notes: session.notes,
    ...
    const groups = groupSessions.map((s) => {
      ...
      return {
        ...
        dateObj: dateObj,
        type: 'group' as const,
        time: s.time || '10:00', // Default if missing
        status: 'scheduled',
  ...
  return (
    <div className="h-[calc(100vh-6rem)] w-full max-w-[1920px] mx-auto flex flex-col lg:flex-row gap-6 p-2 animate-in fade-in duration-500 overflow-hidden">
      {toast && <Toast message={toast.msg} type={toast.type} onClose={() => setToast(null)} />}

      {/* --- LEFT PANEL: THE CALENDAR (Crystal Glass Theme) --- */}
      <div className="flex-1 flex flex-col bg-white/80 backdrop-blur-2xl border border-white/60 shadow-2xl shadow-indigo-100/50 rounded-3xl overflow-hidden relative">
        {/* Luminous Background Accents (Very Subtle) */}
        ...
        <div className="flex items-center justify-between p-6 border-b border-indigo-50/50">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              size="sm"
              onClick={() => onOpenGroupModal('schedule')}
              variant="secondary"
        ...
        {/* CALENDAR GRID (Luminous) */}
        <div className="flex-1 grid grid-cols-7 grid-rows-5 lg:grid-rows-6 p-6 gap-3 overflow-y-auto custom-scrollbar">
          {/* Padding Days */}
          {Array(calendarDays.padding)
            .fill(null)
            .map((_, i) => (
              <div key={`pad-${i}`} className="bg-transparent opacity-0 pointer-events-none"></div>
            ))}
      ...
      {/* --- RIGHT PANEL: TIMELINE (Clean Clinical Look) --- */}
      <div className="w-full lg:w-[400px] shrink-0 flex flex-col gap-6 h-full relative z-20">
        {/* Date Header Card */}
              ...
              {selectedEvents.map((ev, i) => (
                <div
                  ...
                  onClick={() => {
                    if (ev.type === 'individual' && ev.patientId) {
                      ...
                      onOpenGroupModal('evolution', {
                        ...groupSessions.find((g) => g.id === ev.id),
                        date: format(ev.dateObj, 'yyyy-MM-dd'),
                      } as GroupSession);
                    }
                  }}
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-3">
                      <div
                        className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-2 shadow-sm ${ev.type === 'group' ? 'bg-indigo-50 text-indigo-600' : 'bg-pink-50 text-pink-600'}`}
                      >
