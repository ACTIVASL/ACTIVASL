...
export const SystemActivity: React.FC<SystemActivityProps> = ({ activities, isLoading }) => {
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'settings':
        return <Settings2 size={16} className="text-slate-600" />;
      case 'session':
        return <CalendarCheck size={16} className="text-pink-600" />;
      case 'report':
        return <FileSignature size={16} className="text-purple-600" />; // Premium Doc
      case 'finance':
        return <CreditCard size={16} className="text-amber-600" />; // Premium Pay
      case 'security':
        return <ShieldCheck size={16} className="text-indigo-600" />; // Premium Sec
      case 'delete':
        return <FileX size={16} className="text-red-500" />; // Premium Void
      default:
      ...
      <div className="space-y-4 flex-1">
        {isLoading ? (
          ...
          activities.map((item) => (
            ...
            >
              <div className="mt-1 p-2 bg-slate-50 rounded-lg shrink-0">{getIcon(item.type)}</div>
              <div>
