...
export const CognitiveModal: React.FC<CognitiveModalProps> = ({
  ...
  const mocaTotal = MOCA_SECTIONS.reduce(
    (sum, s) => sum + parseInt(String(mocaDetails[s.id] || 0)),
    0,
  );
  const mmseTotal = MMSE_SECTIONS.reduce(
    (sum, s) => sum + parseInt(String(mmseDetails[s.id] || 0)),
    0,
  );
