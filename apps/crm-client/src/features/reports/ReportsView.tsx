  ...
  return (
    <div className="max-w-7xl mx-auto animate-in fade-in space-y-6">
      <header className="flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Informes Clínicos</h1>
        ...
        <div className="hidden md:block text-right">
          ...
          <p className="text-sm text-slate-600">Ve a la ficha del paciente &rarr; Informe</p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredReports.map((report) => (
          <Card
            key={report.id}
            className="p-6 hover:shadow-lg transition-all group cursor-pointer border-l-4 border-l-purple-500"
          >
            ...
            <p className="text-xs text-slate-500 mb-4 font-mono">
              {report.id} • {report.date || 'Sin fecha'}
            </p>
            ...
            <div className="space-y-2 text-sm text-slate-600 mb-6">
              <div className="flex items-center gap-2">
                ...
                <span className="capitalize">
                  {report.type === 'initial' ? 'Evaluación Inicial' : report.type}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <Calendar size={14} />
