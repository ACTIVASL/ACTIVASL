          ...
          {deletedSessions.length === 0 ? (
            ...
            <div className="space-y-4">
              <div className="bg-blue-50 border border-blue-200 p-3 rounded-lg flex gap-3 text-sm text-blue-800 mb-4 items-start">
                ...
                <p>
                  Los elementos aquí pueden ser restaurados. Si eliminas un paciente, todo se
              ...
              {deletedSessions.map((s) => (
                <div
                  key={s.id}
                  className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex justify-between items-center group hover:border-indigo-200 transition-colors"
                >
                  <div className="flex items-center gap-4">
                    <div className="bg-slate-100 p-2.5 rounded-lg text-slate-400 font-bold text-xs text-center min-w-[50px]">
                      ...
                      <span className="uppercase">
                        {new Date(s.date).toLocaleString('es', { month: 'short' }) || ''}
                      </span>
                    </div>
                    <div>
                      ...
                      <p className="text-xs text-slate-400">
                        Eliminado:{' '}
                        {s.deletedAt ? new Date(s.deletedAt).toLocaleDateString() : 'Desconocido'}
                      </p>
                    </div>
                  </div>
                  <Button
